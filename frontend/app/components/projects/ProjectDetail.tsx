...
export const ProjectDetail: React.FC = () => {
  ...
  } = useStages(projectId || undefined);

  // Update the availableStages to include both global and custom stages
  const filteredAvailableStages = allStages.filter(
    (stage) =>
      !project?.stages?.some(
        (projectStage: any) => projectStage.stage._id === stage._id
      )
  );

  // Determine if user can edit this project
  const canEditProject =
    user?.role === "manager" && project?.owner._id === user._id;
  ...
  const handleUpdateSubmit = async (values: UpdateStageValues) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/stages/update-stage/${
          stageToUpdate._id
        }`,
        {
          method: "PUT",
...
<div className="flex items-center justify-between mb-6">
  <div className="flex items-center">
    ...
    <div>
      ...
      <div className="flex items-center mt-1 flex-nowrap">
  ...
  <div className="flex flex-col items-end gap-2">
    ...
    <div className="flex gap-2">
      ...
      {canEditProject &&
        ...
        filteredAvailableStages.length > 0 && (
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-lg">Available Stages</CardTitle>
              <p className="text-sm text-muted-foreground">
                Click on a stage to add it to the project
              </p>
            </CardHeader>
            <CardContent>
            ...
            <div className="flex items-center justify-center text-center">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  This project has been marked as completed. No further changes
                  can be made.
                </p>
              </div>
            </div>
        ...
        <CardContent>
          {stages.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              {canEditProject &&
                project?.status !== "Completed" &&
                " Click on available stages above to add them."}
            </div>
          ) : viewMode === "cards" ? (
              ...
              >
                {connections.map((conn, index) => {
                  const fromElement = document.getElementById(
                    `stage-card-${conn.from_stage._id}`
                  );
                  const toElement = document.getElementById(
                    `stage-card-${conn.to_stage._id}`
                  );
                  ...
                  const containerRect =
                    fromElement.parentElement?.getBoundingClientRect();
                ...
                {stages.map((stage) => (
                  <ProjectStageCard
                    ...
                    onDelete={() =>
                      canEditProject &&
                      project?.status !== "Completed" &&
                      handleDeleteClick(stage._id, false)
                    }
                    onMarkComplete={() =>
                      canEditProject &&
                      project?.status !== "Completed" &&
                      handleMarkComplete(stage._id)
                    }
            ...
            <ProjectStageTable
              ...
              onDelete={(stageId) =>
                canEditProject &&
                project?.status !== "Completed" &&
                handleDeleteClick(stageId, false)
              }
              onMarkComplete={(stageId) =>
                canEditProject &&
                project?.status !== "Completed" &&
                handleMarkComplete(stageId)
              }
              canEdit={canEditProject}
              projectStatus={project?.status}
            />
